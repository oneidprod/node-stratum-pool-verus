...
var PbaasRegistrar = require('./pbaasRegistrar.js');
...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    this.start = function () {
        ...
        SetupDaemonInterface(function () {
            DetectCoinData(function () {
                SetupRecipients();
                SetupJobManager();
                if (options.pbaasChains && options.pbaasChains.length > 0) {
                    _this.pbaasRegistrar = new PbaasRegistrar(options, emitWarningLog);
                }
                OnBlockchainSynced(function () {
                    GetFirstJob(function () {
                        ...
                        StartStratumServer(function () {
                            OutputPoolInfo();
                            _this.emit('started');
                            if (_this.pbaasRegistrar) {
                                setInterval(function () {
                                    _this.daemon.cmd('getmininginfo', [], function(results) {
                                        var r = results[0] && results[0].response;
                                        if (r && r.mergemining === 0) {
                                            emitWarningLog('PBaaS mergemining=0, disconnecting stratum clients to trigger failover');
    ...
    function SetupJobManager() {
        ...
        _this.jobManager.on('newBlock', function (blockTemplate) {
            ...
            // trigger addmergedblock registration for PBaaS chains
            if (_this.pbaasRegistrar) {
                _this.pbaasRegistrar.registerAll();
            }
                ...
                if (jobManagerLastSubmitBlockHex === blockHex) {
                    ...
                    SubmitBlock(shareData.height, blockHex, function () {
                        if (!shareData.blockOnlyPBaaS) {                        
                            ...
                            var candidates = [];
                            if (_this.pbaasRegistrar) {
                                candidates = shareData.pbaasChainMatches || _this.pbaasRegistrar.getMatchingChains(shareData.blockHash);
                                // Chain targets in pbaasRegistrar may be stale vs the job's merged_target.
                                // If the pre-filter returns nothing, verify all chains via getblock anyway.
                                if (candidates.length === 0) {
                                    candidates = _this.pbaasRegistrar.getAllChainNames();
                                }
                                shareData.pbaasChainMatches = candidates;
                            }
                            if (_this.pbaasRegistrar && candidates.length > 0) {
                                _this.pbaasRegistrar.verifyChainAcceptance(shareData.blockHash, candidates, function(acceptedChains) {
                                    shareData.pbaasChainMatches = acceptedChains;
                                    isValidBlock = acceptedChains.length > 0;
                                    emitShare();
                                });
                            } else {
                                isValidBlock = false;
    ...
    function StartStratumServer(finishedCallback) {
        ...
        _this.stratumServer.on('started', function () {
            ...
            client.on('difficultyChanged', function (diff) {
                ...
                var job = _this.jobManager.currentJob.getJobParams().slice();
                job[7] = false;
