...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    function SetupJobManager() {
        ...
        _this.jobManager.on('newBlock', function (blockTemplate) {
            ...
            // notify PBaaS chain daemons of new merged mining work
            if (options.pbaasChains && Array.isArray(options.pbaasChains)) {
                options.pbaasChains.forEach(function (chain) {
                    var auth = 'Basic ' + Buffer.from(chain.user + ':' + chain.password).toString('base64');
                    var body = JSON.stringify({ jsonrpc: '1.0', method: 'addmergedblock', params: [blockTemplate.rpcData.coinbasetxn.data, blockTemplate.rpcData.coinbasetxn.hash] });
                    ...
                    req.on('error', function (e) {
                        emitWarningLog('addmergedblock failed for chain ' + chain.name + ': ' + e.message);
                    });
                    req.write(body);
                    req.end();
                });
