...
function assembleBlockHex(rpcData) {
    ...
    // nTime placeholder - verusd replaces this
    header.writeUInt32LE(rpcData.curtime, pos); pos += 4;
...
module.exports = function PbaasRegistrar(options, emitWarningLog) {
    ...
    function registerChain(chain) {
        rpcCall(chain.host, chain.port, chain.user, chain.password, 'getblocktemplate', [{}], function (err, result) {
            ...
            var hexdata;
            try { hexdata = assembleBlockHex(result.result); }
            catch (e) {
    ...
    var intervals = { vARRR: 10000, CHIPS: 10000, VDEX: 60000 };
