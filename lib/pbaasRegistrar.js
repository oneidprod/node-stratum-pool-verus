...
module.exports = function PbaasRegistrar(options, emitWarningLog) {
        ...
        rpcCall(vrscDaemon.host, vrscDaemon.port, vrscDaemon.user, vrscDaemon.password,
            ...
            function (err, result) {
                if (err || (result && result.error)) {
                    ...
                    console.log('[PbaasRegistrar] addmergedblock error for ' + chain.name + ':', err || result.error);
                } else {
                    var res = result && result.result;
                    console.log('[PbaasRegistrar] addmergedblock ok for ' + chain.name + ':', JSON.stringify(res));
    ...
    function registerChain(chain) {
        console.log('[PbaasRegistrar] calling GBT for ' + chain.name + ' at ' + chain.host + ':' + chain.port);
        rpcCall(chain.host, chain.port, chain.user, chain.password, 'getblocktemplate', [{}], function (err, result) {
            if (err || !result || result.error) {
                ...
                console.log('[PbaasRegistrar] GBT error for ' + chain.name + ':', err || result.error);
                return;
            }
            console.log('[PbaasRegistrar] GBT ok for ' + chain.name + ', calling addmergedblock on ' + vrscDaemon.host + ':' + vrscDaemon.port);
            submitMerged(chain, result.result);
        });
    }
